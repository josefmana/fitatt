...
rprint <- function(x, dec = 2) {
  sprintf(paste0("%.",dec,"f"), round(x, dec))
}

# CALCULATE MEAN & STANDARD DEVIATION ----
msd <- function(x, .dec = 2) {
  paste0(
    rprint(mean(x, na.rm = TRUE), .dec),
    " (", rprint(sd(x, na.rm = TRUE), .dec), ")"
  )
}

# EXTRACT FREQUENCIES ----
frequencies <- function(x, .sep = "/") {
  paste(table(x), collapse = .sep)
}


# DROP LEADING ZERO ----
zerolead <- function(x, .dec = 3, fortext = FALSE) {
  ifelse(
    fortext,
    yes = ifelse(x >= .001, paste0("= ", sub("0.", ".", rprint(x, .dec), fixed = TRUE)), "< .001"),
    no = ifelse(x >= .001, sub("0.", ".", rprint(x, .dec), fixed = TRUE), "< .001")
  )
}
...
extract_stats <- function(.models, labs, type = "continuous") {
  ...
  if (type == "continuous") {
    tab <- sapply(names(.models), function(i) {
      aov <- anova(.models[[i]]) # extract ANOVA table
      eta <- eta_squared(.models[[i]], partial = FALSE, alternative = "two")
      data.frame(
        label = with(labs, label[column == i]),
        Stat = with(aov, {
          paste0("F(", paste(Df, collapse = ", "), ") = ", rprint(na.omit(`F value`), 3))
        }),
        ES = with(eta, {
          paste0("eta_2 = ", rprint(Eta2, 2)," [", rprint(CI_low), ", ", rprint(CI_high), "]")
        }),
        p.value = zerolead(aov$`Pr(>F)`, fortext = FALSE)
      )
    })
    ...
    tab <- sapply(names(.models), function(i) {
      data.frame(
        label = with(labs, label[column == i]),
        Stat = with(.models[[i]], {
          paste0("X2(", parameter, ") = ", rprint(statistic, 3))
        }),
        ES = with(cramers_v(.models[[i]], alternative = "two"), {
          paste0(
            "Cramer's V = ", rprint(Cramers_v_adjusted, 2),
            " [", rprint(CI_low), ", ", rprint(CI_high), "]"
          )
        }),
        p.value = zerolead(.models[[i]]$p.value, fortext = FALSE)
      )
    })
  }
  # reformat and return
...
import_data <- function(file) {
  read.xlsx(xlsxFile = file, sheet = "7IPAQ_GAI_GDS_DRUGS") %>%
    ...
    rename_with(
      .cols = starts_with("SFT_"),
      .fn = ~ sub("SFT_", "", .x)
    ) %>%
    # keep only variables of interest
    select(
      ID, PA_att, # identification and group (predictor) variables
      Age, MMSE, Education, Waist_circumference, BMI, GDS15, GAI, BRI, MET, # descriptive variables
    ...
    mutate(
      ...
      across(
        ...
        .fns = ~ factor(.x, levels = 1:3, ordered = TRUE)
      ),
      across(
...
fit_regressions <- function(.data, .variables, .predictors = "PA_att") {
  labs <- set_names(with(.variables, all_of(column[type == "continuous"])))
  lapply(labs, function(y) {
    lm(formula = as.formula(paste(y, .predictors, sep = " ~ ")), data = .data)
  })
}

# PREPARE CHI-SQUARES FOR NOMINAL VARIABLES ----
compute_chisq <- function(.data, .variables) {
  labs <- set_names(with(.variables, all_of(column[type == "categorical"])))
  lapply(labs, function(y) {
...
prepare_table <- function(.data, .models, .chisquares, .variables) {
  ...
  # put them together
  left_join(desc, stats, by = "label")
}


# PREPARE TABLE WITH POST-HOC COMPARISONS RESULTS ----
table_pairwise <- function(.models, .variables) {
  map_dfr(set_names( names(.models)), function(i) {
    tukey_hsd(.models[[i]]) %>%
      as.data.frame() %>%
      mutate(
...
plot_pairwise <- function(.data, .variables, save = FALSE) {
  # keep only z-score variables
  vars <- .variables[grepl("(z-score)", .variables$label, fixed = TRUE),]
  ...
  d0 <- .data %>%
    ...
    mutate(lab = sapply(outcome, \(i) vars$label[vars$column == i], USE.NAMES = FALSE))
