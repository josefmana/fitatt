...
list(
  ...
  tar_target(
    name = datafile,
    command = here("data-raw", "COSACTIW_DATA_Klara_final.xlsx"),
    format = "file"
  ),
  tar_target(
    name = data,
  ...
  tar_target(
    ...
    command = plot_pairwise(data, variables, save = TRUE)
  ),
  tar_target(
    name = multiple_regressions,
    command = fit_regressions(data, variables, "PA_att + MET + Age + BMI + Education + MMSE")
  )
)
